...
-------------------------------------------------------------------------------
Script Name:     exp_int_13_Script Type 41b.py
Category:        5-Internal Mechanism Validation
Researcher:      Dr. David Swanagon
Original Date:   February 11, 2026
Paper Reference: Section 24.3, Theorem 24.3 (Spectral-Spatial Correspondence)
Theorem Support: "Asymptotic Equivalence of Spectral Mass and Propagator Decay"

-------------------------------------------------------------------------------
EXPERIMENT DESCRIPTION:
    Tests the fundamental identity between the spectral mass gap ($m$) and the 
    spatial decay of the propagator ($G(r)$). In a massive field theory, the 
    correlation between two points must decay exponentially as $e^{-mr}$.
    
    The script constructs a 1D discrete Laplacian, identifies the 
    theoretical spectral mass, and solves for the system's Green's Function 
    using a Conjugate Gradient (CG) solver. By performing linear regression 
    on the logarithmic tail of the propagator (the "Goldilocks Zone"), 
    the experiment verifies if the fitted decay slope matches the 
    analytically predicted spectral mass.

ADVERSARIAL CONDITIONS:
    - Lattice Size:             N = 2000 (To eliminate finite-volume bias)
    - Fitting Region:           r ∈ [200, 600] (Asymptotic limit)
    - Spectral Mass:            $2\sin(\pi / 2(N+1))$
    - Metric:                   Ratio of Fitted Slope to Spectral Mass

PASS CRITERIA:
    1. Propagator Identity:    Ratio (Fit / Mass) ≈ 1.00.
    2. Regression Quality:     $R^2 > 0.9999$.
    3. Numerical Convergence:  Successful CG solution with $rtol < 1e-12$.

USAGE:
    python3 exp_core_val_41b_precision_exponential_decay.py --verbose
