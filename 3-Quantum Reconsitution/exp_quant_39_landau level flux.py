...
EXPERIMENT DESCRIPTION:
    ...
    even in the infinite volume limit. It validates that the vacuum energy
    is lifted strictly by the topological charge (Flux) of the background.

ADVERSARIAL CONDITIONS:
    - Flux:                    Constant Total Flux (20 * pi) across all N
    - Grid Sizes:              20x20 to 60x60
    - Limit:                   Continuum (N -> Infinity)

PASS CRITERIA:
    1. Convergence:            Scaled Energy (N^2 * lambda) converges to Total Flux (~62.8).
    2. Persistence:            The gap does not collapse to zero as the grid refines.
